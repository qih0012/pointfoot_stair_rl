...
class BipedStairs(BaseTask):
    ...
    def _reward_upright_posture(self):
        """直立姿态奖励 - 奖励保持直立"""
        # 基于重力投影，越接近 [0, 0, -1] 越好
        upright_reward = torch.exp(-5.0 * torch.sum(torch.square(self.projected_gravity[:, :2]), dim=1))
        return upright_reward
    
    def _reward_feet_contact_reward(self):
        """双脚接触地面奖励"""
        # 检测双脚是否都接触地面
        foot_contacts = self.contact_forces[:, self.feet_indices, 2] > 1.0  # z方向接触力
        both_feet_contact = foot_contacts.all(dim=1).float()
        single_foot_contact = (foot_contacts.sum(dim=1) == 1).float()
        
        # 双脚接触给最高奖励，单脚接触给部分奖励
        contact_reward = both_feet_contact * 1.0 + single_foot_contact * 0.3
        return contact_reward
    
    def _reward_joint_stability(self):
        """关节稳定性奖励 - 鼓励关节位置接近默认值"""
        # 计算关节位置与默认位置的差异
        joint_pos_error = torch.sum(torch.square(self.dof_pos - self.default_dof_pos), dim=1)
        stability_reward = torch.exp(-2.0 * joint_pos_error)
        return stability_reward
    
    def _reward_com_stability(self):
        """重心稳定奖励 - 基于基座位置的稳定性"""
        # 惩罚基座的快速移动（线性和角速度）
        lin_vel_penalty = torch.sum(torch.square(self.base_lin_vel[:, :2]), dim=1)  # xy平面速度
        ang_vel_penalty = torch.sum(torch.square(self.base_ang_vel), dim=1)  # 所有轴角速度
        
        # 奖励低速度（稳定）
        stability_reward = torch.exp(-1.0 * (lin_vel_penalty + ang_vel_penalty))
        return stability_reward
    
    def _reward_standing_still(self):
        """静止站立奖励 - 奖励在目标高度附近保持静止"""
        # 检查是否在目标高度附近
        height_error = torch.abs(self.root_states[:, 2] - self.env_origins[:, 2] - self.cfg.rewards.base_height_target)
        at_target_height = (height_error < 0.1).float()  # 10cm容差
        
        # 检查是否相对静止
        total_velocity = torch.sum(torch.square(self.base_lin_vel), dim=1) + torch.sum(torch.square(self.base_ang_vel), dim=1)
        is_still = (total_velocity < 0.5).float()  # 速度阈值
        
        # 只有在目标高度且相对静止时才给奖励
        standing_reward = at_target_height * is_still
